...
neighborhoods_near_wells_3km <- holc_neighborhoods %>%
  ...
  select(neighborhood) %>%
  st_centroid()
wells_producing <- wells_pre1935_sf %>% filter(cumulative_boe > 0)
wells_injection <- wells_pre1935_sf %>% filter(production_type == "INJECTION")
wells_oil_gas   <- wells_pre1935_sf %>% filter(production_type == "OIL" | 
                                                 production_type == "GAS" |
                                                 production_type == "OIL & GAS")
wells_unknown   <- wells_pre1935_sf %>% filter(production_type == "UNKNOWN")
...
write_csv(holc_neighborhoods_buffer_1km_wells_all,
          "data/interim/holc_neighborhoods_buffer_1km_wells_all_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_3km_wells_all,
          "data/interim/holc_neighborhoods_buffer_3km_wells_all_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_1km_wells_injection,
          "data/interim/holc_neighborhoods_buffer_1km_wells_injection_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_3km_wells_injection,
          "data/interim/holc_neighborhoods_buffer_3km_wells_injection_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_1km_wells_oil_gas,
          "data/interim/holc_neighborhoods_buffer_1km_wells_oil_gas_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_3km_wells_oil_gas,
          "data/interim/holc_neighborhoods_buffer_3km_wells_oil_gas_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_1km_wells_producing,
          "data/interim/holc_neighborhoods_buffer_1km_wells_producing_pre1935.csv")
...
write_csv(holc_neighborhoods_buffer_3km_wells_producing,
          "data/interim/holc_neighborhoods_buffer_3km_wells_producing_pre1935.csv")


##---------------------------------------------------------------------------
## assesses exposure to unkown wells for each neighborhood
...
write_csv(holc_neighborhoods_buffer_1km_wells_unknown,
          "data/interim/holc_neighborhoods_buffer_1km_wells_unknown_pre1935.csv")
...
holc_neighborhoods_buffer_3km_wells_unknown <-
  tibble(neighborhood = "", 
         buffer_3km_wells_unknown_n = as.numeric(NA))
# loops through exposure assessment function
for(i in c(1:nrow(neighborhoods_near_wells_3km))) {
  holc_neighborhoods_buffer_3km_wells_unknown[i, ] <- 
    assessExposureBufferCount(data_geography = neighborhoods_near_wells_3km[i, ],
                              data_wells     = wells_unknown,
                              buffer_dist_m  = 3000,
                              exp_variable   = "buffer_3km_wells_unknown_n")
  print(i)
}
# exports dataset
write_csv(holc_neighborhoods_buffer_3km_wells_unknown,
          "data/interim/holc_neighborhoods_buffer_3km_wells_unknown_pre1935.csv")
