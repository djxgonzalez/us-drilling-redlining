##============================================================================##
## 2a - adaption of propensity scoring script from A. Nardone for HOLC neighborhoods (Grades A&B)
...
dat <- dplyr::filter(dat, holc_grade == "A" | holc_grade == "B")

#superlearner libraries to run lasso & a screener 
SL.library <- c("SL.glm",  "SL.mean", "SL.bayesglm", "SL.earth", "SL.gam")

#ID your exposure of interest (e.g., propensity to be bluelined vs. greenlined)
dat$exposure <- ifelse(dat$holc_grade=="B", 1, 0)
...
supporteddat<-dat[dat$holc_grade %in% c("A", "B") & dat$cpa1<quantile(dat$cpa1[dat$holc_grade=="A"], probs=.99) & 
                     dat$cpa1>quantile(dat$cpa1[dat$holc_grade=="B"], probs=.01),]
...
datk$Grade<-ifelse(datk$exposure==0, "A", ifelse(datk$exposure==1, "B", 2))
a<-density(datk$score[datk$Grade=="A"])
b<-density(datk$score[datk$Grade=="B"])
plot(a, main="Propensity to be grade B", xlab="Propensity Score", ylim=c(0, 5))
polygon(a, col="cadetblue3")
lines(density(datk$score[datk$Grade=="B"]))
polygon(b, col=rgb(0.9, 0, 0.4, 0.6))


  #ggplot rendition (HOLC grade-specific colors)
  #still want: legend + dark version 
a_vec <- as.vector(datk$score[datk$Grade == "A"]) %>%
  tibble() %>%
  rename("v1" = ".")

b_vec <- as.vector(datk$score[datk$Grade == "B"]) %>%
  tibble() %>%
  rename("v1" = ".")

ps_plot_ab <- ggplot() +
  geom_density(data = a_vec, aes(x = v1), color = "#9da464", fill = "#9da464", alpha = 0.3) + 
  geom_density(data = b_vec, aes(x = v1), color = "#91a8a5", fill = "#91a8a5", alpha = 0.3) + 
  labs(x = "Propensity Score", y = "Density") +
  ggtitle("Propensity for receiving HOLC grade B") +
...
ps_plot_ab
  ...
  #export figure
ggsave(filename = "ps_plot_ab.png", plot = ps_plot_ab, device = "png",
...
datk <- datk %>%
  mutate(holc_num = ifelse(holc_grade == "A", 1, 0))
...
#Grab matched data
propensity_scores_ab <- match.data(ps)

#Save df as rds
saveRDS(propensity_scores_ab, file = "./data/processed/propensity_scores_ab.rds")
