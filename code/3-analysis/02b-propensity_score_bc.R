##============================================================================##
## 2b - adaption of propensity scoring script from A. Nardone for HOLC neighborhoods (Grades B&C)
...
#filter data for only B and C graded neighborhoods for comparison
dat <- dplyr::filter(dat, holc_grade == "B" | holc_grade == "C")

#superlearner libraries to run lasso & a screener 
SL.library <- c("SL.glm",  "SL.mean", "SL.bayesglm", "SL.earth", "SL.gam")

#ID your exposure of interest (e.g., propensity to be yellowlined vs. greenlined)
dat$exposure <- ifelse(dat$holc_grade=="C", 1, 0)
...
supporteddat<-dat[dat$holc_grade %in% c("B", "C") & dat$cpa1<quantile(dat$cpa1[dat$holc_grade=="B"], probs=.99) & 
                     dat$cpa1>quantile(dat$cpa1[dat$holc_grade=="C"], probs=.01),]
...
datk$Grade<-ifelse(datk$exposure==0, "B", ifelse(datk$exposure==1, "C", 2))
b<-density(datk$score[datk$Grade=="B"])
c<-density(datk$score[datk$Grade=="C"])
plot(b, main="Propensity to be grade C", xlab="Propensity Score", ylim=c(0, 5))
polygon(b, col="cadetblue3")
lines(density(datk$score[datk$Grade=="B"]))
polygon(c, col=rgb(0.9, 0, 0.4, 0.6))
...
ps_plot_bc <- ggplot() +
  ...
  ggtitle("Propensity for receiving HOLC grade C") +
...
ps_plot_bc
  ...
  #export figure
ggsave(filename = "ps_plot_bc.png", plot = ps_plot_bc, device = "png",
...
datk <- datk %>%
  mutate(holc_num = ifelse(holc_grade == "B", 1, 0))
...
#Grab matched data
propensity_scores_bc <- match.data(ps)

#Save df as rds
saveRDS(propensity_scores_bc, file = "./data/processed/propensity_scores_bc.rds")
